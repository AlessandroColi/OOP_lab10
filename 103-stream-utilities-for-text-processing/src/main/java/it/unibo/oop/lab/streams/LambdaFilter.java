...
public final class LambdaFilter extends JFrame {
    ...
    private enum Command {
        ...
        COUNT_WORDS("Write the count for each word", e -> Arrays.stream(e.split(ANY_NON_WORD))
        		.collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
        		.entrySet().stream()
        		.map(c -> c.getKey() + " -> " + c.getValue())
        		.collect(Collectors.joining("\n")) ),
        
        SORT("List all the words in alphabetical order", e -> Arrays.stream(e.split(ANY_NON_WORD))
        		.sorted()
        		.collect(Collectors.joining("\n"))  ),
        
    	TO_LOWER("Convert to lowercase", e -> e.toLowerCase() );
